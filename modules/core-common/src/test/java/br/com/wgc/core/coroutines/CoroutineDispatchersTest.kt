...
class CoroutineDispatchersTest {
    ...
    @Test
    fun retryWithBackoff_whenSucceedsOnFirstAttempt_shouldReturnResultImmediately() = runTest {
        var attempts = 0
    ...
    @Test
    fun retryWithBackoff_whenFailsOnceThenSucceeds_shouldRetryAndReturn() = runTest {
        var attempts = 0
        val result = retryWithBackoff<String>(
            times = 3,
            initialDelayMs = 10L
    ...
    @Test
    fun retryWithBackoff_whenAllAttemptsExhausted_shouldThrowLastException() = runTest {
        var attempts = 0
        try {
